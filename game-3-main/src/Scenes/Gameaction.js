class Gameaction extends Phaser.Scene {
    ...
    create() {
      ...
      let healthText = this.add.text(16, 64, 'Health: 3', { fontSize: '32px', fill: "black" }).setScrollFactor(0);
      ...
      this.anims.get('walk').on('animationcomplete', function (animation, frame) {
        if (animation.key === 'walk' && frame.index === frame.total - 1) {
          this.sound.play("walk_sound");
        }
      });
